...
func _physics_process(delta: float) -> void:
	if entered and PipeCompletion.s_e:
		GameManager.leave_scene()
		SaveManager.promote_temp_to_permanent()

		get_tree().change_scene_to_file("res://scenes/floors/secret_ending.tscn")

func _on_secret_end_body_entered(body: CharacterBody2D) -> void:
	entered = true

func _on_secret_end_body_exited(body: CharacterBody2D) -> void:
	entered = false
