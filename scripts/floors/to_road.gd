...
func _on_to_road_body_entered(body: CharacterBody2D) -> void:
	entered = true

func _on_to_road_body_exited(body: CharacterBody2D) -> void:
	entered = false
...
func _physics_process(delta: float) -> void:
	if entered:
		...
		get_tree().change_scene_to_file("res://scenes/floors/road.tscn")
