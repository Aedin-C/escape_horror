...
func _on_end_body_entered(body: CharacterBody2D) -> void:
	entered = true

func _on_end_body_exited(body: CharacterBody2D) -> void:
	entered = false

func _physics_process(delta: float) -> void:
	if entered:
		GameManager.set_scene("res://scenes/floors/road_ending.tscn")
