...
func _on_up_atticback_body_entered(body: CharacterBody2D):
	entered = true

func _on_up_atticback_body_exited(body: CharacterBody2D):
	entered = false
...
func _physics_process(delta: float):
	if entered and Input.is_action_just_pressed("right_click"):
		...
		get_tree().change_scene_to_file("res://scenes/floors/attic_back2.tscn")
